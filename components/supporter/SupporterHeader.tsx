...
import { MotionLink } from '../_shared/MotionLink'
        ...
        <div className="flex items-center gap-1.5 sm:gap-x-3 min-w-0">
          ...
          {event && event.isPublic && (
            <MotionLink
              ...
              aria-label={`View ${event.title}${event.status === 'ONGOING' ? ' — tickets on sale now' : ''}`}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="relative p-2 dark:bg-neutral-800 dark:border-neutral-700 dark:hover:bg-neutral-700 bg-neutral-100 border-neutral-200 hover:bg-neutral-200 border rounded-lg transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/50 h-9 w-9 flex items-center justify-center shrink-0"
            >
              <span className="suit text-xl" aria-hidden="true">
                ♠
              </span>
              {event.status === 'ONGOING' && (
                <span
                  className="absolute -top-1 -right-1 w-2 h-2 bg-emerald-500 rounded-full animate-pulse"
                  aria-hidden="true"
                />
              )}
            </MotionLink>

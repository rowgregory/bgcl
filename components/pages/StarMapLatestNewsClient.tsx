...
export const StarMapLatestNewsClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    if (data?.id) {
      await updatePageBySlug('latest-news', content)
      store.dispatch(
        showToast({
      ...
      await createPage('latest-news', content)
      store.dispatch(
        showToast({
          message: 'Page created successfully',
          description: 'Your latest news page text is now accessible to all visitors',
          type: 'success'
        })
      )
    }

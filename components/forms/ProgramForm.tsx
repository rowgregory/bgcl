            ...
            {inputs?.showThemes && (
              <div className="space-y-4">
                ...
                {themes && themes.length > 0 && (
                  <div className="p-4 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg">
                    <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-3">
                      Add from Theme Library
                    </label>

                    {/* Only show themes not already connected */}
                    {themes.filter((t) => !((inputs?.themes as { id: string }[]) ?? []).some((pt) => pt.id === t.id))
                      .length > 0 ? (
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {themes
                          .filter((t) => !((inputs?.themes as { id: string }[]) ?? []).some((pt) => pt.id === t.id))
                          .map((theme) => (
                            <label
                              key={theme.id}
                            ...
                            >
                                ...
                                <input
                                  ...
                                  onChange={() => {
                                    store.dispatch(
                                      setInputs({
                                        formName: 'programForm',
                                        data: { themes: [...(inputs.themes as any[]), theme] }
                                      })
                                    )
                                  }}
                              ...
                              <button
                                ...
                                className="p-1 bg-red-500/80 hover:bg-red-500 rounded text-white shrink-0"
                              >

...
/** Full-width switch card used by the payment toggles. */
export function ToggleCard({ checked, onChange, Icon, title, description }: Props) {
  return (
    <button
      type="button"
      role="switch"
        ...
        <div className="min-w-0 flex-1">
          <p
            className={`text-xs sm:text-sm font-medium truncate ${
              checked ? 'dark:text-white text-neutral-900' : 'dark:text-zinc-300 text-neutral-700'
            }`}
          >
            {title}
          </p>
          <p className="text-[10px] sm:text-xs dark:text-zinc-500 text-neutral-500 truncate">{description}</p>
        </div>
      </div>

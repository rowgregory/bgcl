...
export function TicketCheckoutForm({ savedCards, setStep }: Props) {
  ...
  const [salesStarted, setSalesStarted] = useState(() => {
    const d = items[0]?.ticketSalesStartDate
    return !d || new Date(d).getTime() <= Date.now()
  })
  const [salesEnded, setSalesEnded] = useState(() => {
    const d = items[0]?.ticketSalesEndDate
    return Boolean(d && new Date(d).getTime() < Date.now())
  })

  useEffect(() => {
    const start = items[0]?.ticketSalesStartDate
    const end = items[0]?.ticketSalesEndDate
    if (!start && !end) return
    ...
    const check = () => {
      ...
      setSalesStarted(!start || new Date(start).getTime() <= now)
      setSalesEnded(Boolean(end && new Date(end).getTime() < now))
    }

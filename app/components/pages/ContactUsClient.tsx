        ...
        <div className="max-w-334 mx-auto space-y-12 sm:space-y-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-12">
            ...
            >
              ...
              {contactInfo.map((info, index) => {
                ...
                return (
                  <a
                    key={index}
                    href={info.href}
                    className="block p-4 sm:p-6 rounded-lg sm:rounded-xl hover:border-sky-500/50 transition-colors dark:bg-neutral-900/50 bg-neutral-50/50"
                  >
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg dark:bg-sky-500/10 bg-sky-100 flex items-center justify-center shrink-0">
                        <Icon className="w-5 h-5 sm:w-6 sm:h-6 dark:text-sky-400 text-sky-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xs sm:text-sm font-semibold dark:text-neutral-500 text-neutral-600 uppercase tracking-wider mb-1">
                          {info.label}
                        </h3>
                        <p className="dark:text-white text-neutral-900 font-semibold group-hover:dark:text-sky-400 group-hover:text-sky-600 transition-colors whitespace-pre-line text-sm sm:text-base">
                          {info.value}
                        </p>
                      </div>
                    </div>
              ...
              <div className="dark:bg-neutral-900 bg-white dark:border-neutral-800 border-neutral-200 p-4 sm:p-6 rounded-lg sm:rounded-xl border">
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xs sm:text-sm font-semibold dark:text-neutral-500 text-neutral-600 uppercase tracking-wider mb-2 sm:mb-3">
                      Fall Hours
                    </h3>
              ...
              {/* Get Involved Card */}
              <div className="dark:bg-neutral-900 bg-white dark:border-neutral-800 border-neutral-200 p-4 sm:p-6 rounded-lg sm:rounded-xl border">
                <h3 className="text-xs sm:text-sm font-semibold dark:text-neutral-500 text-neutral-600 uppercase tracking-wider mb-3 sm:mb-4">
                  Get Involved
                </h3>
                <div className="space-y-2 sm:space-y-3">
                  <button
                    onClick={() => store.dispatch(setOpenVolunteerDrawer())}
                    className="w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg dark:bg-sky-500/10 bg-sky-50 dark:hover:bg-sky-500/20 hover:bg-sky-100 transition-colors group"
                  >
                    <Heart className="w-4 h-4 sm:w-5 sm:h-5 dark:text-sky-400 text-sky-600" />
                    <span className="dark:text-white text-neutral-900 font-medium text-sm sm:text-base">
                      Become a Volunteer
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 dark:text-sky-400 text-sky-600 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>
                  <Link
                    href="/get-involved"
                    className="w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg dark:bg-indigo-500/10 bg-indigo-50 dark:hover:bg-indigo-500/20 hover:bg-indigo-100 transition-colors group"
                  >
                    <Briefcase className="w-4 h-4 sm:w-5 sm:h-5 dark:text-indigo-400 text-indigo-600" />
                    <span className="dark:text-white text-neutral-900 font-medium text-sm sm:text-base">
                      Join Our Team
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 dark:text-indigo-400 text-indigo-600 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
                  </Link>
                </div>
              </div>
            </motion.div>

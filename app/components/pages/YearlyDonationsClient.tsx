...
export default function YearlyDonationsClient({ yearlyDonations }: { yearlyDonations: any }) {
  const [searchQuery, setSearchQuery] = useState('')

  const filteredDonations = yearlyDonations.filter((donation: { name: string; email: string }) => {
    const matchesSearch =
      searchQuery === '' ||
  ...
  const totalDonations = yearlyDonations.reduce((sum, d) => sum + d.totalAmount * 100, 0)
        ...
        <div className="mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-neutral-900 dark:text-white">{yearlyDonations.length}</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          {filteredDonations.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm">Yearly donations will appear here</p>
            </motion.div>
          ) : (
            <motion.div
              className="space-y-2"

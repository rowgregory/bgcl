...
export default function SqyshCard({ member }: SqyshCardProps) {
  ...
  const { closeButtonRef } = useOverlayAccessibility(isActive)
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (i < fullText.length) {
        ...
        setTimeout(() => {
          setShowButtons(true)
          // Move focus to first button once it appears
  ...
  const handleCardClick = () => {
    if (member.isSqysh) setIsActive(true)
  }
  ...
  return (
    <>
      <div
        ...
        aria-label="Learn about Sqysh web development services"
        aria-haspopup="dialog"
        aria-expanded={isActive}
        className="relative shrink-0 h-92 overflow-visible dark:bg-neutral-800 bg-neutral-100 rounded-2xl cursor-pointer group focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2"
      >
        <Picture
          src={member.image}
          alt="Sqysh mascot"
      ...
      <AnimatePresence>
        {isActive && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Sqysh web development services"
            aria-describedby="sqysh-dialog-description"
            onKeyDown={handleKeyDown}
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-130 overflow-hidden"
          >
            {/* Screen reader description */}
            <p id="sqysh-dialog-description" className="sr-only">
              Sqysh is the web development company that built this site. This dialog asks if you are interested in their
              services.
            </p>
            ...
            <motion.button
              ...
              onClick={handleClose}
              aria-label="Close Sqysh dialog"
              className="absolute top-3 right-3 sm:top-6 sm:right-6 w-8 h-8 sm:w-10 sm:h-10 bg-black/20 backdrop-blur-md rounded-full border border-neutral-600/30 flex items-center justify-center text-neutral-300 hover:text-white hover:border-neutral-400/50 transition-all z-10 touch-manipulation focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
            ...
            <div className="relative hidden md:block">
              ...
              >
                <div className="bg-black/40 backdrop-blur-xl rounded-2xl border border-teal-600/30 shadow-2xl p-6">
                  <p
                    ...
                    aria-live="polite"
                    aria-label={isTyping ? undefined : fullText}
                  >
                    {displayedText}
                    {isTyping && (
                      <span aria-hidden="true" className="inline-block w-2 h-6 bg-white ml-1 animate-pulse" />
                    )}
                  </p>
                  ...
                  {showButtons && (
                    ...
                    >
                      <ActionButton
                        ...
                        onClick={handleYes}
                        title="Yes"
                        description="I want my site to be sqysh'd"
                        dotColor="bg-teal-500"
                      />
                      <ActionButton
                        onClick={handleClose}
                        title="No"
                        description="This site is awesome, but maybe another time."
              ...
              >
                ...
                <a
                  ...
                  aria-label="Visit Sqysh website - opens in a new tab"
                  className="text-neutral-400 text-sm hover:text-teal-400 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 rounded"
                >
                  www.sqysh.io
                </a>
              </motion.div>
                ...
                <div className="bg-black/40 backdrop-blur-xl rounded-xl sm:rounded-2xl border border-teal-600/30 shadow-2xl p-4 xs:p-5 sm:p-6">
                  ...
                  <div className="mb-4 pb-4 border-b border-teal-500/20">
                    ...
                    <a
                      ...
                      rel="noopener noreferrer"
                      aria-label="Visit Sqysh website - opens in a new tab"
                      className="text-neutral-400 text-xs xs:text-sm hover:text-teal-400 transition-colors underline focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 rounded"
                    >
                      www.sqysh.io
                    </a>
                  </div>
                  ...
                  <p
                    className="text-base xs:text-lg sm:text-xl font-bold text-white mb-4 xs:mb-5 sm:mb-6 min-h-10"
                    aria-live="polite"
                    aria-label={isTyping ? undefined : fullText}
                  >
                    {displayedText}
                    {isTyping && (
                      <span
                        aria-hidden="true"
                        className="inline-block w-1.5 h-5 xs:w-2 xs:h-6 bg-white ml-1 animate-pulse"
                      />
                    )}
                  </p>

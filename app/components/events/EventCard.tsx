...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="group dark:bg-neutral-800/50 dark:backdrop-blur dark:border-neutral-700/50 bg-white border-neutral-200 border rounded-lg overflow-hidden dark:hover:border-neutral-600 hover:border-neutral-300 transition-all"
    >
      {/* Header */}
          ...
          <div className="space-y-2">
            {event.tickets.slice(0, 2).map((ticket) => (
              <div key={ticket.id} className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold dark:text-sky-400 text-sky-600">${(ticket.price / 100).toFixed(2)}</span>
              </div>
            ))}

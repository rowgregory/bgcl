export const HIDDEN_PATHS = [
  '/auth',
  '/admin',
  '/supporter',
  '/checkout',
  '/order-confirmation',
  '/programs/',
  '/donate',
  '/campaigns/',
  '/program/',
  '/events/',
  '/super'
]

export const mainNavigationLinks = [
  { label: 'HOME', href: '/', priority: 1 },
  { label: 'ABOUT', href: '/about', priority: 1 },
  { label: 'TEAM', href: '/team', priority: 1 },
  { label: 'PROGRAMS', href: '/programs', priority: 1 },
  { label: 'CAMPAIGNS', href: '/campaigns', priority: 2 },
  { label: 'EVENTS', href: '/events', priority: 3 },
  { label: 'AWARD WINNERS', href: '/award-winners', priority: 4 },
  { label: 'LATEST NEWS', href: '/latest-news', priority: 5 },
  { label: 'PARTNERSHIPS', href: '/partners', priority: 6 },
  { label: 'GET INVOLVED', href: '/get-involved', priority: 7 },
  { label: 'CONTACT', href: '/contact', priority: 7 }
]

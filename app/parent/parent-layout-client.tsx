...
import { parentNavLinks } from "../lib/navigation/parentNavLinks";

const ParentLayoutClient: FC<ILayoutClient> = ({ data, children }) => {
  const [isNavigationCollapsed, setIsNavigationCollapsed] = useState(false);
  const path = useCustomPathname();
  //   const session = useSession();
  const selectedPage = getCurrentPageId(path, parentNavLinks);
  ...
  useEffect(() => {
    if (selectedPage) {
      ...
      console.log("PARENT LAYOUT CLIENT: ", data);
    }
  }, [selectedPage, data]);
...
export default ParentLayoutClient;

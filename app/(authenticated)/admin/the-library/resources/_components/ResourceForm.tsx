...
import { ResourceFormInput } from '@/lib/validations/resource.validation'
import { useResourceDrawer } from '@/stores/drawers'
import { motion } from 'framer-motion'
import { AlertCircle } from 'lucide-react'
import { useFormContext } from 'react-hook-form'

export default function ResourceForm({ isUpdating }) {
  const {
    register,
    formState: { errors, isSubmitting }
  } = useFormContext<ResourceFormInput>()
    ...
    <div className="w-full mx-auto pb-40">
      ...
      >
        <h1 className="text-3xl md:text-4xl font-black dark:text-white text-neutral-900">
          {isUpdating ? 'Edit Resource' : 'Create Resource'}
        </h1>
        <p className="dark:text-neutral-400 text-neutral-600">Configure your resource details</p>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            <label htmlFor="url" className="block text-sm font-medium dark:text-neutral-300 text-neutral-700 mb-2">
              Url *
            </label>
            <input
              type="text"
              ...
              placeholder=""
            />
            {errors?.url && <p className="mt-2 text-sm text-red-400">{errors.url.message}</p>}
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-neutral-950 border-t border-neutral-200 dark:border-neutral-800 p-6 md:p-8 z-40">
        <div className="max-w-2xl mx-auto flex flex-col md:flex-row gap-4">
          ...
          >
            {isSubmitting ? 'Saving...' : isUpdating ? 'Update resource' : 'Create resource'}
          </motion.button>
        </div>
      </div>
    </div>

...
export const PageEditorEventsClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('event', content) : await createPage('event', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your event page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

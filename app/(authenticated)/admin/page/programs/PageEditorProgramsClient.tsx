...
export const PageEditorProgramsClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('program', content) : await createPage('program', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your program page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

...
export const PageEditorCapitalCampaignClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('capital', content) : await createPage('capital', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your capital campaign page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

...
export const PageEditorTeamClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('team', content) : await createPage('team', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your team page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

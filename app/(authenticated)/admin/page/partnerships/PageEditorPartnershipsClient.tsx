...
export const PageEditorPartnershipsClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('partner', content) : await createPage('partner', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your partnership page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

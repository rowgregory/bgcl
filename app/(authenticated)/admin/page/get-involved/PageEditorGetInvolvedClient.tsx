...
export const PageEditorGetInvolvedClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('get-involved', content) : await createPage('get-involved', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your get involved page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

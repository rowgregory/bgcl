...
import { PageContentEditor } from '@/app/(authenticated)/admin/page/_components/PageContentEditor'
...
export const PageEditorCITApplicationClient = ({ data }) => {
  ...
  const handleSave = async (content: any): Promise<void> => {
    ...
    try {
      const res = isUpdate ? await updatePageBySlug('cit', content) : await createPage('cit', content)
      ...
      setMessage({
        ...
        description: isUpdate
          ...
          : 'Your CIT application page text is now accessible to all visitors'
      })
    } catch (error: unknown) {
      setMessage({
        type: 'error',

...
import PaymentMethodModal from '../../_components/PaymentMethodModal'
...
export function PaymentMethods({ savedCards }) {
  ...
  const confirmRemove = async (cardId: string) => {
    await handleDeletePaymentMethod(cardId)
      ...
      >
        ...
        {savedCards && savedCards.length > 0 ? (
          <ul role="list" aria-label="Saved payment methods" className="space-y-2">
            {savedCards.map((card) => {
              const isBusy = settingDefault === card.id || deletingPaymentMethod === card.id
              const isConfirming = confirmingRemove === card.id
                ...
                >
                  ...
                  {isConfirming ? (
                    <div className="flex items-center gap-3 shrink-0">
                      <p role="alert" className="text-xs dark:text-neutral-300 text-neutral-700">
                        Remove this card?
                      </p>
                      <button
                        type="button"
                        ...
                        disabled={isBusy}
                        className="text-xs font-medium dark:text-neutral-400 text-neutral-500 dark:hover:text-white hover:text-neutral-900 transition-colors disabled:opacity-50"
                      >
                        Cancel
                      </button>
                      <button
                        type="button"
                        onClick={() => confirmRemove(card.id)}
                        disabled={isBusy}
                        autoFocus
                        className="flex items-center gap-1 text-xs font-semibold text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors disabled:opacity-50"
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button
                        type="button"
                        onClick={() => setConfirmingRemove(card.id)}
                        disabled={isBusy}
